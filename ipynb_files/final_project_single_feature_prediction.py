...
FEATURES   = 1     # currently Close-only
BATCH_SIZE = 32
...
# For now, use Close-only (easy); later upgrade to all 5 features
data = df_daily[["Close"]].copy()
...
class TransformerPredictor(nn.Module):
    """
    Transformer-based multi-step forecasting model.

    - The input is projected into a higher-dimensional transformer embedding
    - Transformer encoder learns relationships between all timesteps via self-attention
    - The final embedding represents the entire historical window
    - Fully-connected layer predicts all future timesteps at once

    NOTE: This version is encoder-only (no autoregressive decoder),
          because parallel forecasting is faster for short horizons
    """

    def __init__(self, number_of_input_features, prediction_length,
                 transformer_embedding_dimension = 32,
        ...
        # One or more transformer encoder layers
        ...
        # Fully-connected layer maps final transformer embedding
        # into prediction_length × number_of_input_features outputs
        self.fully_connected_layer = nn.Linear(
            transformer_embedding_dimension,
            prediction_length * number_of_input_features
    ...
    def forward(self, input_sequence):
        ...
        # Step 1: project into embedding dimension
        projected_input = self.input_projection_layer(input_sequence)   # (B, seq_len, d)

        # Step 2: transformer expects shape (seq_len, batch_size, d)
        projected_input = projected_input.permute(1, 0, 2)

        # Step 3: run through transformer encoder
        encoded_sequence = self.transformer_encoder(projected_input)   # (seq_len, B, d)

        # Step 4: final timestep embedding represents whole historical window
        final_embedding = encoded_sequence[-1]                         # (B, d)

        # Step 5: fully-connected layer → multi-step forecast
        prediction_vector = self.fully_connected_layer(final_embedding)

        # Step 6: reshape into forecast tensor
        forecast = prediction_vector.view(
...
transformer_model, transformer_train_loss, transformer_val_loss = train_model(
...
def plot_test_forecast(
    ...
    plt.plot(future_actual, label="True Future", marker='o')
    plt.plot(predicted_actual, label="Predicted Future", marker='x')
